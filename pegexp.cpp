...
struct PegexState {
	const char*	pc;		// Current location in the pegex
	const char*	text;		// Current text we're looking at
	const char*	target;		// Original start of the string
	bool		(*callout)(const char* pegex, const char*& text);
};

int		pegex_match(const char *pegex, const char*& text, bool (*callout)(const char* pegex, const char*& text) = 0);
bool		pegex_match_here(PegexState& state);

static int	unhex(char c);
static uint32_t	escaped_char(const char*& pc);
static bool	char_class(PegexState& state);
static bool	match_atom(PegexState& state);
static bool	match_alternate(PegexState& state);
static bool	match_repeat(PegexState& state, int min, int max);
static const char* skip_atom(const char*& pegex);

/*
 * Search for pegex anywhere in text.
 *
 * Return the matched length, with text pointing to the start of the match.
 * If no match is found, text is set to NULL and the returned length is -1
 */
int pegex_match(
	const char *pegex,
	const char*& text,
	bool (*callout)(const char* pegex, const char*& text)	// Will be used for subexpression calls, etc
)
{
	const char*	trial = text;		// The first search starts here
	PegexState	state = {
			.pc = pegex,		// The expression to match
...
/*
 * Search for pegex at beginning of text given
 */
bool pegex_match_here(PegexState& state)
{
...
/*
 * Unpack a character, including special character escapes, hex/unicode escapes, etc.
 * Advance pc as necessary
 */
static uint32_t	escaped_char(const char*& pc)
{
	uint32_t	rc = *pc++;
...
/*
 * Match a character class, possibly negated
 */
static bool char_class(PegexState& state)
{
	bool	negated;
...
static bool match_atom(PegexState& state)
{
	...
	{
		const char*	revert = state.text;	// We always return to the current text
...
/*
 * Repeatedly match the current atom at least min and at most max times
 */
static bool match_repeat(PegexState& state, int min, int max)
{
	const char*	iter_start;
	const char*	repeat_pc = state.pc;
	int		repetitions = 0;
	...
	{
		const char*	iter_start = state.text;
		state.pc = repeat_pc;
		if (!match_atom(state))
		{
...
static const char* skip_atom(const char*& pc)
{
	switch (*pc++)
	{
	case '\\':	// Escaped literal char
...
main(int argc, const char** argv)
{
	...
	for (const char** subject = argv+2; subject < argv+argc; subject++)
	{
		...
		int	length = pegex_match(argv[1], text);
		fprintf(stdout, "%s\t%s\t", argv[1], *subject);
		if (length >= 0)
			fprintf(stdout, "+%d\t%.*s\n", (int)(text - *subject), length, text);
		else
